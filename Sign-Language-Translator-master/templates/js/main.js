...
const start = async() => {
    ...
    const totalClassesDisplay = document.getElementById("total-classes");
    ...
    const updateStats = () => {
        totalClassesDisplay.textContent = classes.length;
        let totalImages = 0;
        classes.forEach(cls => totalImages += cls.count);
    ...
    const addClass = () => {
        ...
        trainingCards.innerHTML += `
            <div class="training-class-card">
                <h4><i class="fas fa-hand-paper"></i> ${Classname}</h4>
                <div class="class-stats">
                    <div class="class-count">
                        <span id="images-${identity}">0</span> images
                    </div>
                    <div class="class-actions">
                        <button class="class-action-btn add-images-btn" id="${identity}">
                            <i class="fas fa-plus"></i>
                            Add Images
                        </button>
                        <button class="class-action-btn delete-class-btn" onclick="deleteClass(${identity})">
...
// Function to delete a training class
function deleteClass(classId) {
    if (confirm('Are you sure you want to delete this class? This action cannot be undone.')) {
        ...
        // Remove from DOM
        const classCard = document.getElementById(classId.toString()).closest('.training-class-card');
        if (classCard) {
            classCard.remove();
        }
